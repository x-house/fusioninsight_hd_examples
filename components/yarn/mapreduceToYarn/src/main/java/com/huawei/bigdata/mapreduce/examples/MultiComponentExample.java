...
/**
 * This is the Mapreduce example that operate multi-components. This example, when in map phase, read hdfs file and get
 * name of male person, then read one row of HBase data and then query Hive to get related data, then concatenate all
 * values. In reduce phase, just save the map output to HBase and HDFS
 * <p>
 * The purpose is mainly guide developer how to add configuration resource and how to do security authentication when
...
public class MultiComponentExample
{
    ...
    // security info
    private static final String PRINCIPAL = "test@HADOOP.COM";

    private static final String KEYTAB = MultiComponentExample.class.getClassLoader().getResource("user.keytab")
            .getPath();

    private static final String KRB = MultiComponentExample.class.getClassLoader().getResource("krb5.conf").getPath();

    private static final String JAAS = MultiComponentExample.class.getClassLoader().getResource("jaas_mr.conf")
            .getPath();
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        job.getCredentials().addToken(new Text("hiveserver2ClientToken"), hive2Token);
